...
packageChaincode() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode package mycc_${VERSION}.tar.gz --path ${CC_SRC_PATH} --lang ${CC_RUNTIME_LANGUAGE} --label mycc_${VERSION} >&log.txt
  res=$?
  set +x
  cat log.txt
...
installChaincode() {
  ...
  setGlobals $PEER $ORG
  set -x
  peer lifecycle chaincode install mycc_${VERSION}.tar.gz >&log.txt
  res=$?
  set +x
  cat log.txt
...
queryNewInstalled(){
  ...
  set +x
  cat log.txt
  # `sed -n '/Package/{s/^Package ID: //; s/, Label:.*$//; p;}' log.txt`
  NEW_PACKAGE_ID=`sed -n '/mycc_${VERSION}/{s/^Package ID: //; s/, Label:.*$//; p;}' log.txt`
  verifyResult $res "Query installed on peer${PEER}.org${ORG} has failed"
  PACKAGE_ID=${NEW_PACKAGE_ID}
  echo NewPackageID is ${NEW_PACKAGE_ID}
  echo 
  echo "===================== Query installed successful on peer${PEER}.org${ORG} on channel ===================== "
  echo
}
...
chaincodeQuery() {
  ...
  do
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["QueryWallet","abc"]}' >&log.txt
    res=$?
    set +x
...
ccQuery() {
  ...
  set -x
  peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["QueryWallet","d8adb453-8775-46ba-ba16-cd1d5c844662"]}' >&log.txt
  res=$?
  set +x
...
chaincodeInvoke() {
  ...
  if [ "${IS_INIT}" -eq "1" ]; then
    ...
    # CCARGS='{"Args":["GetToken","abc","apple","12"]}'
    # CCARGS='{"Args":["DeleteWallet","bcd"]}'
    # CCARGS='{"Args":["GetToken","abc","apple","12"]}'
    # 创建钱包
    # CCARGS='{"function":"CreateWallet","Args":["d8adb453-8775-46ba-ba16-cd1d5c844662"]}'
    CCARGS='{"function":"GetToken","Args":["d8adb453-8775-46ba-ba16-cd1d5c844662","apple","1"]}'
    INIT_ARG=""
  fi
